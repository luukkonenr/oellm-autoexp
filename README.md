...
> **REMOVED on this branch.** The `oellm_evals`-based chain (the `oellm_eval`
> backend, the `submodules/oellm_evals` submodule, `scripts/install_eval_env.sh`,
> `scripts/prefetch_datasets.py`, and the `config/experiments/korbi/chain_*`
> configs) is **not used here and has been removed from this branch.** For
> conversion and evaluation use the standalone path below instead. This section
> is retained only for reference on branches that still ship `oellm_evals`.
...
> **REMOVED on this branch.** The `titan_oellm` (torch-titan) backend is **not
> used here and the submodule has been removed from this branch.** These
> instructions apply only to branches that still ship the titan submodule.
